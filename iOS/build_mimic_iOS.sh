...
./configure --prefix=$TOP_DIR/install_arm64 CFLAGS="-Ofast -mios-version-min=5.0" LDFLAGS="-flto" CC="xcrun -sdk iphoneos clang -arch arm64" --host=arm
make clean && make && make install

# build armv7
echo Building arch armv7

./configure --prefix=$TOP_DIR/install_armv7 CFLAGS="-Ofast -mios-version-min=5.0" LDFLAGS="-flto" CC="xcrun -sdk iphoneos clang -arch armv7" --host=arm
make clean && make && make install
...
./configure --prefix=$TOP_DIR/install_armv7s CFLAGS="-Ofast -mios-version-min=5.0" LDFLAGS="-flto" CC="xcrun -sdk iphoneos clang -arch armv7s" --host=arm
make clean && make && make install

# build simulator i386 and x86_64
echo Building arch i386

./configure --prefix=$TOP_DIR/install_i386 CFLAGS="-Ofast -mios-version-min=5.0" LDFLAGS="-flto" CC="xcrun -sdk iphonesimulator clang -arch i386"
make clean && make && make install

echo Building arch x86_64

./configure --prefix=$TOP_DIR/install_x86_64 CFLAGS="-Ofast -mios-version-min=5.0" LDFLAGS="-flto" CC="xcrun -sdk iphonesimulator clang -arch x86_64"
make clean && make && make install
